...
# devtools: always show the "Inspect Element" context menu entry, independent of
# Titanium/upstream's kAndroidDevToolsFrontend runtime feature gate. Clicking it still
# reaches the same native ContextMenuNativeDelegateImpl::InspectElement() ->
# DevToolsWindow::InspectElement() call (see context_menu_native_delegate_impl.cc),
# which is unconditional on BUILDFLAG(ENABLE_DEVTOOLS_FRONTEND) and does not depend on
# kAndroidDevToolsFrontend at all -- that runtime feature only ever gated this menu
# item's visibility, not the underlying devtools invocation.
sed -i 's|boolean shouldShowDeveloperMenu() {|boolean shouldShowDeveloperMenu() { return true;|' chrome/android/java/src/org/chromium/chrome/browser/contextmenu/ChromeContextMenuPopulator.java
...
# ext: install from local .crx/.zip file (crbug.com/406136787-adjacent: Android
# has no drag-and-drop from a file manager, and "Load unpacked" only accepts a
# directory via SAF, so there was previously no way to install an already-packed
# extension picked from local storage). Adds developerPrivate.installExtensionFile(),
# reusing the same ui::SelectFileDialog flow as loadUnpacked() and the same crx/zip
# install dispatch as installDroppedFile() -- see patches/crx_import/*.inc.
mkdir -p titanium/crx_import
cp $SCRIPT_DIR/patches/crx_import/developer_private_install_extension_file_decl.inc titanium/crx_import/
cp $SCRIPT_DIR/patches/crx_import/developer_private_install_extension_file_impl.inc titanium/crx_import/
sed -i 's|^class DeveloperPrivateNotifyDragInstallInProgressFunction|#include "titanium/crx_import/developer_private_install_extension_file_decl.inc"\n\nclass DeveloperPrivateNotifyDragInstallInProgressFunction|' chrome/browser/extensions/api/developer_private/developer_private_functions.h
sed -i 's|^DeveloperPrivateNotifyDragInstallInProgressFunction::$|#include "titanium/crx_import/developer_private_install_extension_file_impl.inc"\n\nDeveloperPrivateNotifyDragInstallInProgressFunction::|' chrome/browser/extensions/api/developer_private/developer_private_functions.cc
sed -i 's|static Promise<undefined> installDroppedFile();|&\n\n  // Titanium: install a user-selected .crx/.zip file (see installDroppedFile\n  // above; this is its non-drag-and-drop counterpart for Android).\n  static Promise<undefined> installExtensionFile();|' chrome/common/extensions/api/developer_private.webidl
sed -i 's|installDroppedFile(): void;|&\n  installExtensionFile(): void;|' chrome/browser/resources/extensions/service.ts
perl -0pi -e 's/(installDroppedFile\(\) \{\n\s*chrome\.developerPrivate\.installDroppedFile\(\);\n\s*\})/$1\n\n  installExtensionFile() {\n    chrome.developerPrivate.installExtensionFile();\n  }/' chrome/browser/resources/extensions/service.ts
sed -i 's|loadUnpacked(): Promise<boolean>;|&\n  installExtensionFile(): void;|' chrome/browser/resources/extensions/toolbar.ts
perl -0pi -e 's/(loadUnpacked\(\) \{\n\s*return Promise\.resolve\(true\);\n\s*\})/$1\n  installExtensionFile() {}/' chrome/browser/resources/extensions/toolbar.ts
perl -0pi -e 's/(protected onLoadUnpackedClick_\(\) \{.*?\n  \})/$1\n\n  protected onInstallExtensionFileClick_() {\n    this.delegate.installExtensionFile();\n    chrome.metricsPrivate.recordUserAction(\x27Options_InstallExtensionFile\x27);\n  }/s' chrome/browser/resources/extensions/toolbar.ts
perl -0pi -e 's/(<cr-button \?hidden="\$\{!this\.canLoadUnpacked_\(\)\}" id="loadUnpacked"\n\s*\@click="\$\{this\.onLoadUnpackedClick_\}">\n\s*\$i18n\{toolbarLoadUnpacked\}\n\s*<\/cr-button>)/$1\n    <cr-button ?hidden="\${!this.canLoadUnpacked_()}" id="installExtensionFile"\n        \@click="\${this.onInstallExtensionFileClick_}">\n      Install extension file\n    <\/cr-button>/' chrome/browser/resources/extensions/toolbar.html.ts
